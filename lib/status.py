...
def status(work: str, verbose: bool = False) -> int:
    ...
    if os.path.isdir(mods):
        ...
        total = sum(os.path.getsize(os.path.join(mods, f)) for f in sb)
        print()
        ...
        for f in sb:
            tag = f"  {G}<- added here{O}" if f in made else ""
            print(f"    {f:<24} {_human(os.path.getsize(os.path.join(mods, f))):>10}{tag}")
