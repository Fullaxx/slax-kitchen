...
# boot appended to --record. $STUB_LEAK_QB makes it leave a qmp directory behind the way
# qemu_boot.py would, by asking tempfile for one and never removing it.
...
printf '{"path": "%s", "iso_name": "%s", "iso_bytes": 1, "accel": "kvm", "markers": [], "missing": [], "result": "pass"}\n' \
    "$path" "$(basename "$iso")" >> "$record"
'''

# Found by tier-c.sh's tool check, never run for anything that matters here.
STUB_TOOLS = {
...
def run_tier_c(tmp, env_extra):
    ...
    p = subprocess.run(
        ["sh", os.path.join(repo, "ci", "tier-c.sh"), "--iso", iso, "--target", "stub-target",
         "--paths", "kernel", "--out", out, "--ledger", os.path.join(tmp, "ledger.json"),
